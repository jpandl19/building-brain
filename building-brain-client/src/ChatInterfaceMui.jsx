...
const ChatInterface = (props) => {
    ...
    const fetchInitialMessage = async () => {
      try {
        const response = await apiClient.get(
          `/api/chat?platformId=${foundPlatformId}`
        );
        ...
        introHelperMessage = 
        `Hello and welcome to BuildingGPT<br /><br /> BuildingGPT makes it simple and easy to troubleshoot and maintain your commercial real estate building. You can do things like:<br />
                    <ul>
  ...
  const sendMessage = async () => {
    ...
    setMessages([
      ...messages,
      { text: inputMessage, sender: "user", feedback: null, intro: false },
    ]);
    setInputMessage("");
    ...
    try {
      ...
      if (
        responseLengthClean == null ||
        responseLengthClean === "" ||
        _.isNaN(responseLengthClean) ||
        _.isEmpty(responseLengthClean)
      ) {
        responseLengthClean = DEFAULT_RESPONSE_LENGTH;
      }

      const response = await apiClient.post(
        `/api/chat?platformId=${staticPlatformId}`,
        {
          message: inputMessage,
          max_response_length: responseLengthClean,
          platformId,
        }
      );
  ...
  const renderFeedbackButtons = (message) => {
    ...
    if (message.feedback != null && message.feedback.sentiment === "positive") {
      ...
      message.feedback.sentiment === "negative"
    ) {
      content = (
        <Box sx={{ paddingLeft: 2 }}>
      ...
      content = (
        <>
          <Grid item xs={3}>
            <Button
              variant="text"
              onClick={() => sendUserFeedback(message, `positive`)}
            >
              <ThumbUpOffAltOutlinedIcon
                sx={{
                  color: `#808080`,
          ...
          <Grid item xs={3}>
            <Button
              variant="text"
              onClick={() => sendUserFeedback(message, `negative`)}
            >
              <ThumbDownOutlinedIcon
                sx={{
                  color: `#808080`,
  ...
  const renderThinkingMessage = () => (
    ...
    >
      <Typography sx={{ ...fontStyles }} variant={`h6`}>
        Thinking...
      </Typography>
    </Box>
  );
  ...
  const renderSendButton = () => (
    <Button
      onClick={sendMessage}
      sx={{ ml: 1, ...fontStyles, backgroundColor: theme.palette.primary.alt }}
      variant="contained"
    >
      Send
    </Button>
  ...
  if (allowedToChat == false) {
    return (
      ...
      >
        <Grid item>
          <Typography sx={{ ...fontStyles }} variant={`h6`}>
            You must be an active subscriber to access this feature
          </Typography>
        </Grid>
      </Grid>
    );
  ...
  const renderReferences = (message) => {
        ...
        >
          ...
          >
            {reference.documentName != null && (
              <Grid item xs={4}>
                <Typography sx={{ ...fontStyles }} variant={`body2`}>
                  File: {reference.documentName}
                </Typography>
              </Grid>
            )}
    ...
    return (
      <Accordion>
        ...
        <AccordionDetails>{rendMessages}</AccordionDetails>
      </Accordion>
    );
  };
      ...
      >
        <Grid item direction={`column`} xs={12} md={12}>
          ...
          <Grid item xs={12}>
            ...
            <Stack
              sx={{ flexGrow: 1, height: `83vh`, overflowY: "auto", p: 2 }}
              spacing={1}
            >
